...
const app=createApp({
  setup(){
    ...
    const saveResultDlg=ref(false),saveResult=ref({})
    ...
    const loadCfg=async()=>{cfgLoading.value=true;try{const{data}=await axios.get('/api/config');cfg.value=data;
      ...
      cfg.value.seedance.default_duration=cfg.value.seedance.default_duration??5}catch(e){toastError(e,'加载配置失败')};cfgLoading.value=false}
    ...
    const CFG_KEEP={
      ...
      seedance:new Set(['model_version','resolution','aspect_ratio','generate_audio','default_duration']),
    ...
    const saveCfg=async()=>{cfgSaving.value=true;try{const body=pruneCfg(cfg.value);const{data}=await axios.put('/api/config',body);saveResult.value=data;saveResultDlg.value=true}catch{saveResult.value={ok:false,message:'保存失败'};saveResultDlg.value=true};cfgSaving.value=false}
    const resetCfg=async()=>{try{await ElementPlus.ElMessageBox.confirm('确定从备份恢复？','确认',{type:'warning'})}catch{return};cfgResetting.value=true;try{const{data}=await axios.post('/api/config/reset');cfg.value=data.config;ElementPlus.ElMessage.success(data.message)}catch{ElementPlus.ElMessage.error('恢复失败')};cfgResetting.value=false}
    ...
    return{
      ...
      cfg,cfgLoading,cfgSaving,cfgResetting,templates,openPanels,saveResultDlg,saveResult,tplDlg,tplDlgContent,loadCfg,saveCfg,resetCfg,viewTpl,onTplImported,
      comfyOnline,
      // Review & 重做
